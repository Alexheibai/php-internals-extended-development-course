...
![PHPHEAP内存池](./image/2-1heap_struct.png)
...
struct _zend_mm_chunk {
	...
	uint32_t           free_tail;               /* 数据块结尾的连续空闲页数 number of free pages at the end of chunk */
...
start_memory_manager()->alloc_globals_ctor(&alloc_globals)->zend_mm_init()：
```code
